...
namespace NeoFPS
{
	...
	public class InputCharacterMotion : CharacterInputBase
    {
        ...
        [SerializeField] float m_VerticalRotationLimit = 60.0f; // Adjust as needed
        [SerializeField] float m_HorizontalRotationLimit = 90.0f;
                ...
                if (Input.touchCount > 0)
                {
                    ...
                    switch (touch.phase)
                    {
                        case TouchPhase.Began:
                            ...
                            break;
                        // Example for touch input
                        case TouchPhase.Moved:
                            Vector2 deltaPosition = touch.position - m_LastTouchPosition;

                            m_RotationX -= deltaPosition.y * m_RotationSpeed * Time.deltaTime;
                            m_RotationX = m_RotationX / 2;
                            m_RotationX = Mathf.Clamp(m_RotationX, -m_VerticalRotationLimit, m_VerticalRotationLimit);

                            m_RotationY += deltaPosition.x * m_RotationSpeed * Time.deltaTime;
                            m_RotationY = m_RotationY / 2;
                            m_RotationY = Mathf.Clamp(m_RotationY, -m_HorizontalRotationLimit, m_HorizontalRotationLimit);

                            m_Aimer.HandleMouseInput(new Vector2(deltaPosition.x, deltaPosition.y));
                            m_Aimer.HandleAnalogInput(new Vector2(deltaPosition.x, deltaPosition.y));
                            break;
                    }
